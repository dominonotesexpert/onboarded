    ...
    <div className={`flex h-full ${minHeightClass} bg-midnight/70 rounded-3xl border border-white/10 overflow-hidden shadow-card relative`}>
      ...
      {showPalette && interactive ? (
        <button
          ...
          className="lg:hidden fixed bottom-4 left-4 z-50 rounded-full bg-gradient-to-r from-blue-500 to-purple-500 text-white p-4 shadow-glow"
          ...
          <>
            <div className="pointer-events-auto absolute top-3 right-3 flex flex-wrap gap-1.5 sm:gap-2 z-20 max-w-[calc(100%-1.5rem)]">
              <button
                type="button"
                onClick={undo}
                className="text-[10px] sm:text-xs text-white/80 bg-white/10 border border-white/15 rounded-full px-2 sm:px-3 py-1 hover:bg-white/15 whitespace-nowrap"
              >
                Undo
              </button>
              <button
